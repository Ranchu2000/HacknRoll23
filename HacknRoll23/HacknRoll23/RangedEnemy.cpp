...
void RangedEnemy::initVariables()
{
	this->clock.restart();
...
void RangedEnemy::initAI()
{
}

void RangedEnemy::initGUI()
{
	this->hpBar.setFillColor(sf::Color::Red);
	this->hpBar.setSize(sf::Vector2f(60.f, 10.f));
}

void RangedEnemy::initAnimations() {
...
RangedEnemy::RangedEnemy(float x, float y, sf::Texture& texture_sheet, Player& player)
	: Enemy(player)
{
	this->initVariables();
	this->initGUI();
	...
	this->createMovement(300.f, 1000.f, 400.f);
...
void RangedEnemy::updateAnimation(const float& dt)
{
	if (this->isAttacking)
	{
...
void RangedEnemy::update(const float& dt, sf::Vector2f& mouse_pos_view, const sf::View& view)
{
...
void RangedEnemy::render(sf::RenderTarget& target, sf::Shader* shader, const sf::Vector2f light_position, const bool show_hitbox)
{
	if (shader)
	{
